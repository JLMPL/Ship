    ...
    : Layer(layerStack)
{
    ...
    m_rect.setPosition({32, 1080 - (5 + 32 + HealthBarHeight + 5)});
    m_rect.setFillColor(sf::Color::White);

    m_hpBack.setSize({256, HealthBarHeight});
    m_hpBack.setPosition({32, 1080 - (HealthBarHeight + 32)});
    m_hpBack.setFillColor({64,64,64,255});

    m_heatBack.setSize({256, 5});
    m_heatBack.setPosition({32, 1080 - (5 + 32 + HealthBarHeight + 5)});
    m_heatBack.setFillColor({64,64,64,255});

    m_heat.setSize({256, 5});
    m_heat.setPosition({32, 1080 - (5 + 32 + HealthBarHeight + 5)});
